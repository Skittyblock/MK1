...
-(void)saveScriptsDict{
    NSError *jError; 
    NSData *jsonData = [NSJSONSerialization dataWithJSONObject:self.scriptsDict options:NSJSONWritingPrettyPrinted error:&jError];
    if(jError){
        alertError([jError localizedDescription]);
        return;
    }
    ...
    NSError *wError;
    [jsonData writeToFile:@"/Library/MK1/scripts.json" options:NSDataWritingAtomic error:&wError];
...
-(void)shareScript:(MKScript *)script description:(NSString *)description{
    [self confirmationDialogWithTitle:@"Share script publicly?" message:@"This will upload the script to a public URL where it can be shared with anyone.\n\nBy continuing you confirm you have permission to upload the content, and that it does not contain any personal information." block:^(BOOL choice){
        ...
        // the same domain is in the appdelegate
        NSMutableURLRequest *request = [NSMutableURLRequest requestWithURL:[NSURL URLWithString:@"https://XXX.REPLACEME.XXX/s/api/new"] 
        ...
        [[[NSURLSession sharedSession] dataTaskWithRequest:request completionHandler:^(NSData *data, NSURLResponse *response, NSError *error){
            ...
            UIPasteboard.generalPasteboard.URL = [NSURL URLWithString:[NSString stringWithFormat:@"https://XXX.REPLACEME.XXX/s/%@", [[NSString alloc] initWithData:data encoding:NSUTF8StringEncoding]]];
            dispatch_async(dispatch_get_main_queue(), ^{
                [hud hide];
                UIAlertController *alert = [UIAlertController alertControllerWithTitle:@"Script Uploaded"
                            message:@"A public URL to share the script has been copied to the clipboard" preferredStyle:UIAlertControllerStyleAlert];
                UIAlertAction* okAction = [UIAlertAction actionWithTitle:@"Ok" style:UIAlertActionStyleDefault
                    handler:^(UIAlertAction * action) {}];
                [alert addAction:okAction];
                [self presentViewController:alert animated:YES completion:nil];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    UIAlertAction *runAction = [UIAlertAction actionWithTitle:@"Run Script" style:UIAlertActionStyleDefault
        handler:^(UIAlertAction * action) {
            static CPDistributedMessagingCenter *c = nil;
            c = [CPDistributedMessagingCenter centerNamed:@"com.castyte.mk1"];
            rocketbootstrap_distributedmessagingcenter_apply(c);
    ...
    UIAlertAction *nameAction = [UIAlertAction actionWithTitle:@"Change Script Name" style:UIAlertActionStyleDefault
		handler:^(UIAlertAction * action) {
    ...
    UIAlertAction *triggerAction = [UIAlertAction actionWithTitle:@"Change Script Trigger" style:UIAlertActionStyleDefault
        handler:^(UIAlertAction * action) {
            [self textBoxWithTitle:@"Script Trigger" message:nil initText:self.scripts[indexPath.row].trigger block:^(NSString *trigger){
                dispatch_async(dispatch_get_main_queue(), ^{
                    ...
                    UIAlertController *alert = [UIAlertController alertControllerWithTitle:@"Trigger Updated"
                                message:@"A respring may be required for changes to take effect" preferredStyle:UIAlertControllerStyleAlert];
                    UIAlertAction* okAction = [UIAlertAction actionWithTitle:@"Ok" style:UIAlertActionStyleDefault
                        handler:^(UIAlertAction * action) {}];
                    [alert addAction:okAction];
                    [self presentViewController:alert animated:YES completion:nil];
    ...
    UIAlertAction *shareAction = [UIAlertAction actionWithTitle:@"Share Script" style:UIAlertActionStyleDefault
        handler:^(UIAlertAction * action) {
